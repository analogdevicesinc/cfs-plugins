...
describe("Unit test for MSDK Single Core UART Loopback", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-uart-loop-back/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-uart-loop-back/.cfsplugin";
