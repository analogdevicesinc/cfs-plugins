...
const filePath = "plugins/dist/msdk-single-core-coremark/.cfsplugin";
...
describe("MSDK Single Core Coremark Tests", () => {
  ...
  describe("GenerateWorkspace_BasicFlow_ShouldGenerateExpectedFiles", () => {
    const expectedWorkspaceFilesBase = [
      ...
      "m4/src/core_portme.c",
      "m4/src/core_portme.h",
      "m4/project.mk",
      "m4/Makefile",
    ...
    Object.entries(supportedSocs).forEach(([soc, boards]) => {
      boards.forEach((board) => {
        it(`For ${soc} soc with ${board} board`, async () => {
          ...
          cfsWorkspace = {
            location:
              "tests/unit-tests/plugins/msdk-single-core-coremark/data",
