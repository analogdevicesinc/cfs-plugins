...
describe("Unit test for Zephyr Single Core LittleFS", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-littlefs/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-littlefs/.cfsplugin";
