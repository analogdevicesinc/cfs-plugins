...
describe("Unit test for MSDK Single Core Library Generate", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-library-generate/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-library-generate/.cfsplugin";
