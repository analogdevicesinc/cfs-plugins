...
describe("Unit test for Zephyr Single Core CRC", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-crc/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-crc/.cfsplugin";
