...
const filePath = "plugins/dist/msdk-single-core-rtc-backup/.cfsplugin";
...
describe("MSDK Single Core RTC Backup Tests", () => {
    ...
    Object.entries(supportedSocs).forEach(([soc, boards]) => {
      boards.forEach((board) => {
        it(`For ${soc} soc with ${board} board`, async () => {
          ...
          cfsWorkspace = {
            location:
              "tests/unit-tests/plugins/msdk-single-core-rtc-backup/data",
