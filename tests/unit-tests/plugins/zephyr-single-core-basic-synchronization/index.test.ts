...
describe("Unit test for Zephyr Single Core Basic Synchronization", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location:
      "tests/unit-tests/plugins/zephyr-single-core-basic-synchronization/data",
  ...
  before(async () => {
    try {
      const filePath =
        "plugins/zephyr-single-core-basic-synchronization/.cfsplugin";
