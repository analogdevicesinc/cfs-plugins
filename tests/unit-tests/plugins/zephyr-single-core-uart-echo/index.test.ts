...
describe("Unit test for Zephyr Single Core UART Echo", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-uart-echo/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-uart-echo/.cfsplugin";
