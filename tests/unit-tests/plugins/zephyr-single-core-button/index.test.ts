...
describe("Unit test for Zephyr Single Core Button", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-button/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-button/.cfsplugin";
