...
describe("Unit test for SHARC-FX single core tflm example template", () => {
  ...
  before(async () => {
    try {
      const filePath = "plugins/sharcfx-single-core-tflm/.cfsplugin";
  ...
  it("Should generate a workspace", async () => {
    ...
    // Confirm that the tflite file was generated
    const paths = {
      tflite: joinPath(
    ...
    expect(tfliteExists, `Expected ${paths.tflite} to exist`).to.be.true;
