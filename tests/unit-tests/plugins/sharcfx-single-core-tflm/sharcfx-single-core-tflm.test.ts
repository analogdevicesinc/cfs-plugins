...
const filePath = "plugins/dist/sharcfx-single-core-tflm/.cfsplugin";
...
describe("SHARC-FX Single Core with TFLM Tests", () => {
  ...
  describe("GenerateWorkspace_BasicFlow_ShouldGenerateExpectedFiles", () => {
    const expectedWorkspaceFilesBase = [
      ...
      "sfx/Makefile",
      "sfx/README.md",
      "sfx/hello_world_f32.tflite",
      "sfx/project.mk",
      "sfx/src/adi_tflm/adi_tflm.hpp",
      "sfx/src/adi_tflm/hello_world_model_f32.cpp",
      "sfx/src/adi_tflm/hello_world_model_f32.hpp",
    ...
    Object.entries(supportedSocs).forEach(([soc, boards]) => {
      boards.forEach((board) => {
        it(`Using ${soc} SoC with ${board} board`, async () => {
          ...
          cfsWorkspace = {
            location: "tests/unit-tests/plugins/sharcfx-single-core-tflm/data",
  ...
  describe("GenerateCode_BasicFlow_ShouldGenerateExpectedFiles", () => {
    const expectedFilesByCoreId: Record<string, string[]> = {
      FX: ["src/system/soc_init.c", "src/linker/memmap.xmm", "aimodels.mk"],
