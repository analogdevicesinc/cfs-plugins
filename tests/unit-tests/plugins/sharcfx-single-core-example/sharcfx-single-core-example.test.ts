...
describe("Unit test for SHARC-FX single core example template", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/sharcfx-single-core-example/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/sharcfx-single-core-example/.cfsplugin";
