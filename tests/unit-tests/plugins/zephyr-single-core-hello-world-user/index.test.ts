...
describe("Unit test for Zephyr Single Core Hello World User", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location:
      "tests/unit-tests/plugins/zephyr-single-core-hello-world-user/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-hello-world-user/.cfsplugin";
