...
describe("Unit test for Zephyr Single Core ADC Sequence", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-adc-sequence/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-adc-sequence/.cfsplugin";
