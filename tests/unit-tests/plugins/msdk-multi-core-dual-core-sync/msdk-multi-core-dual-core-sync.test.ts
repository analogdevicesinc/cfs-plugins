...
const filePath = "plugins/dist/msdk-multi-core-dual-core-sync/.cfsplugin";
...
const projects = [
  { name: "m4", path: "./m4" },
  { name: "riscv", path: "./riscv" },
];

// Expected generated files keyed by cfsconfig ProjectId
const expectedCodeGenFilesByProjectId: Record<string, string[]> = {
  CM4: ["soc_init.c", "soc_init.h", "config.mk", "memory.ld"],
  RV: ["soc_init.c", "soc_init.h", "config.mk", "memory.ld"],
};

describe("MSDK Multi Core Dual Core Sync Tests", () => {
  ...
  describe("GenerateWorkspace_BasicFlow_ShouldGenerateExpectedFiles", () => {
    const expectedWorkspaceFilesBase = [
      ...
      "m4/m4.jdebug",
      "riscv/.vscode/flash.gdb",
      "riscv/README.md",
      "riscv/.vscode/settings.json",
      "riscv/.vscode/c_cpp_properties.json",
      "riscv/src/main.c",
      "riscv/project.mk",
      "riscv/Makefile",
      "riscv/.vscode/cfs.tasks.json",
    ...
    Object.entries(supportedSocs).forEach(([soc, boards]) => {
      boards.forEach((board) => {
        it(`For ${soc} soc with ${board} board`, async () => {
          ...
          // The RISC-V launch config is not generated for MAX32690.
          if (soc.toLowerCase() !== "max32690") {
            expectedWorkspaceFiles.push("riscv/.vscode/launch.json");
          }

          cfsWorkspace = {
            location:
              "tests/unit-tests/plugins/msdk-multi-core-dual-core-sync/data",
