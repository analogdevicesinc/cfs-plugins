...
describe("Unit test for MSDK Multicore Dual Core Sync", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-multi-core-dual-core-sync/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-multi-core-dual-core-sync/.cfsplugin";
