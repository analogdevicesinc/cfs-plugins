...
describe("Unit test for MSDK Single Core RTC", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-rtc/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-rtc/.cfsplugin";
