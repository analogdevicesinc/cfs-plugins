...
describe("Unit test for MSDK Single Core I2C Scan", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-i2c-scan/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-i2c-scan/.cfsplugin";
