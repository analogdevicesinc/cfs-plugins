...
describe("Unit test for Zephyr Single Core Counter Alarm", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-counter-alarm/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-counter-alarm/.cfsplugin";
