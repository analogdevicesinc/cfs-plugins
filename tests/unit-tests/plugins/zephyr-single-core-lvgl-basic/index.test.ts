...
describe("Unit test for Zephyr Single Core LVGL basic", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-lvgl-basic/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-lvgl-basic/.cfsplugin";
