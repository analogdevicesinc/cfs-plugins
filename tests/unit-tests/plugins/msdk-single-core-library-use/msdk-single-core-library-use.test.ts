...
describe("Unit test for MSDK Single Core Library Use", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-library-use/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-library-use/.cfsplugin";
