...
describe("Unit test for Zephyr Single Core Eventfd", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-eventfd/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-eventfd/.cfsplugin";
