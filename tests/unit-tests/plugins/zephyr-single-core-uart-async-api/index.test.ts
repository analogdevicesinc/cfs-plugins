...
describe("Unit test for Zephyr Single Core UART ASYNC API", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-uart-async-api/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-uart-async-api/.cfsplugin";
