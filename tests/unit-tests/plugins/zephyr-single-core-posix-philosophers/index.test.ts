...
describe("Unit test for Zephyr Single Core posix Philosophers", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location:
      "tests/unit-tests/plugins/zephyr-single-core-posix-philosophers/data",
  ...
  before(async () => {
    try {
      const filePath =
        "plugins/zephyr-single-core-posix-philosophers/.cfsplugin";
