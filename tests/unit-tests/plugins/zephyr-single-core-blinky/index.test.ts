...
describe("Unit test for Zephyr Single Core Blinky", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-blinky/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-blinky/.cfsplugin";
