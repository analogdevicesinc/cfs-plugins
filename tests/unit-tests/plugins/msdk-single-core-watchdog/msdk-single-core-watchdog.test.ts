...
describe("Unit test for MSDK Single Core Watchdog", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-watchdog/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-watchdog/.cfsplugin";
