...
describe("Unit test for Zephyr Single Core Watchdog", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-watchdog/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-watchdog/.cfsplugin";
