...
describe("Unit test for Zephyr Single Core I2C Custom Target", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-i2c-custom-target/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-i2c-custom-target/.cfsplugin";
