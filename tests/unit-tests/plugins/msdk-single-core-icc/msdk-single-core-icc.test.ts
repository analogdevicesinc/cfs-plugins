...
describe("Unit test for MSDK Single Core ICC", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-icc/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-icc/.cfsplugin";
