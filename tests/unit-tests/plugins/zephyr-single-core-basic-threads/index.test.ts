...
describe("Unit test for Zephyr Single Core Basic Thread Manipulation", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/zephyr-single-core-basic-threads/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/zephyr-single-core-basic-threads/.cfsplugin";
