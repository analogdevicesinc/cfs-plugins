...
describe("Unit test for MSDK Single Core Hello World C++", () => {
  let plugin: GenericPlugin;
  let pluginInfo: CfsPluginInfo;
  const cfsWorkspace = {
    location: "tests/unit-tests/plugins/msdk-single-core-hello-world-cpp/data",
  ...
  before(async () => {
    try {
      const filePath = "plugins/msdk-single-core-hello-world-cpp/.cfsplugin";
