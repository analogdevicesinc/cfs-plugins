...
After flashing and launching the example, an LED on the board will blink once every second. This is the application waiting for user Push Button to be pressed (PB1/SW2 on MAX78000EVKIT, SW1 on MAX78000FTHR), and gives a window for a serial terminal to be connected. After connecting the serial terminal, the application will output the following contents for MAX78000EVKIT (similar to MAX78000FTHR):

```
***** Flash Control Example *****
Press Button PB1/SW2 to continue...
...
At this point, the "magic" and test pattern values have been written to flash. Press reset button to reset the board, after which the application will restart. Press the user button again to continue the application, which will print out the following contents:
