...
int write_test_pattern(void)
{
    ...
    if (err) {
        printf("Failed to erase page at address 0x%x with error code %i\n", TEST_ADDRESS, err);
        return err;
    }
    ...
    for (uint32_t addr = TEST_ADDRESS + 4; addr < TEST_ADDRESS + MXC_FLASH_PAGE_SIZE; addr += 4) {
        /*  
            ...
            byte-addressable.  Increment the address by 4 bytes to write in 32-bit 
            chunks.  
...
int main(void)
{
    ...
    // Select the correct user-button prompt for the current board variant.
#if defined(BOARD_EVKIT_V1)
    printf("Press Button PB1/SW2 to continue...\n\n");
#else
    printf("Press Button SW1 to continue...\n\n");
#endif
