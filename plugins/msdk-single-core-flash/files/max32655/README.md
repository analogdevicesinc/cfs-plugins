...
If using the MAX32655FTHR (FTHR_Apps_P1):
...
After flashing and launching the example, an LED on the board will blink once every second. This is the application waiting for the user Push Button to be pressed (SW3 on MAX32655EVKIT, SW2 on MAX32655FTHR), and gives a window for a serial terminal to be connected. After connecting the serial terminal, the application will output the following contents for MAX32655EVKIT (similar to MAX32655FTHR):

```
***** Flash Control Example *****
Press Button SW3 to continue...

---(Critical)---
Erasing page 64 of flash (addr 0x1007e000)...
...
At this point, the "magic" and test pattern values have been written to flash. Press reset button to reset the board, after which the application will restart. Push user button again to continue the application, which will print out the following contents:
