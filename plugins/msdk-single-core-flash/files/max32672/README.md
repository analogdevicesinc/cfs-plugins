...
After flashing and launching the example, an LED on the board will blink once every second. This is the application waiting for Push Button to be pressed (SW3 on MAX32672EVKIT, SW2 on MAX32672FTHR), and gives a window for a serial terminal to be connected. After connecting the serial terminal, the application will output the following contents for MAX32672EVKIT (similar to MAX32672FTHR):
...
---(Critical)---
Erasing page of flash (addr 0x1007e000)...
...
At this point, the "magic" and test pattern values have been written to flash. Press reset button to reset the board, after which the application will restart. Press the push button again to continue the application, which will print out the following contents:
