...
int main(void)
{
    ...
    // Get the input clock frequency used by the UART for data transmission (WRITING_UART)
    int uart_freq = MXC_UART_GetFrequency(WRITING_UART);
    ...
    printf("\nConnect the TX pin of UART%d to the RX pin of UART%d for this example.\n",
           WRITING_UART_IDX, READING_UART_IDX);
