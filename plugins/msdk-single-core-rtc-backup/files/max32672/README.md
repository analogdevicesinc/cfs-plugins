...
The RTC time-of-day alarm is used to wake the device from backup mode every TIME\_OF\_DAY\_SEC seconds (defined at the top of main.c). When the device wakes up, it will print the current time of the RTC to the terminal window, re-arm the time-of-day alarm, return to backup mode and wait for the next wakeup signal from the RTC.
...
If using the MAX32672EVKIT (EvKit\_V1):
