...
void RTC_IRQHandler(void)
{
    ...
    if (flags & MXC_RTC_INT_FL_LONG) {
        ...
        MXC_RTC_GetSeconds(&time); // busy already checked in MXC_RTC_DisableInt
...
int main(void)
{
    ...
    printf("The RTC is enabled and the sub-second alarm set to trigger every %d ms.\n",
           SUBSECOND_MSEC_0);
    printf("LED D1 is toggled each time the sub-second alarm triggers.\n\n");
    ...
    printf("triggers, the rate of the sub-second alarm is switched to %d ms.\n\n",
           SUBSECOND_MSEC_1);
    printf("LED D2 is toggled each time the time-of-day alarm triggers.\n\n");
