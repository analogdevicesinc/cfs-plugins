...
This example demonstrates the use of the HW CRC calculator. The example first generates a CRC over 100 32-bit words (400 bytes). The resulting CRC is inverted and placed at the end of the 100 words. Another CRC is then calculated over the resulting 101 words. The final result is compared with the expected value. This sequence is repeated twice - once using the blocking CRC functions and once using the non-blocking (asynchronous) functions.
